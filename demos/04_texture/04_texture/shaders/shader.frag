...
vec4 effect_waves(vec2 uv) {
    ...
    vec2 uv2 = uv;
    uv2.y += sin(uv.x * freq + time * speed) * amp;

    return texture(tex0, uv2);
}
...
vec4 effect_zoom(vec2 uv) {
    float zoom = 1.0 + sin(time) * 0.3;
    vec2 centered = (uv - 0.5) / zoom + 0.5;
    return texture(tex0, centered);
}
...
void main() {
    ...
    switch (mode) {
        case 0: result = effect_zoom(tex_coord); break;
        case 1: result = effect_grayscale(tex_coord); break;
        case 2: result = effect_invert(tex_coord); break;
        case 3: result = effect_sepia(tex_coord); break;
        case 4: result = effect_color_adjust(tex_coord); break;
        case 5: result = effect_vignette(tex_coord); break;
        case 6: result = effect_waves(tex_coord); break;
        case 7: result = effect_scroll(tex_coord); break;
        default: result = effect_original(tex_coord); break;
    }
