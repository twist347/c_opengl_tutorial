...
#define TEXTURE1_SRC    "textures/img1.png"
#define TEXTURE2_SRC    "textures/img2.png"
...
static void render(GLFWwindow *window, GLuint shader_program, GLuint VAO, GLuint *textures);
...
int main(void) {
    ...
    textures[0] = load_texture(TEXTURE1_SRC);
    if (!textures[0]) {
        exit_code = EXIT_FAILURE;
        goto cleanup;
    }

    textures[1] = load_texture(TEXTURE2_SRC);
    if (!textures[1]) {
        exit_code = EXIT_FAILURE;
        goto cleanup;
    }
